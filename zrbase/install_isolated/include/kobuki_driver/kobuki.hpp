...
public:
  ...
  ** Packet Processing
  *******************************************/
  void sendBaseControlCommand_to_kinco();
  int32_t  getUploadDateFromRespondBuf(unsigned char * pRespondBuf);
  double computeDistance(int32_t pluse_old, int32_t pluse_new);
  void set_CM_SpeedAndRadium_Kinco(int32_t speed, int32_t radium);
  void get_CurrentDistance(unsigned char * lrec, unsigned char * rrec);
  void disable_CM_Motor_Kinco();
  void enable_CM_Motor_Kinco();
  void set_CoordinationMode_Kinco();
  bool sendCommandToKinco(unsigned char * cmd, unsigned int size, unsigned char * respondbuf);
  unsigned char generateCheckByte(unsigned char * buf);
  bool readSerial(unsigned char * pRespondBuf, unsigned int size);
  void spin();
