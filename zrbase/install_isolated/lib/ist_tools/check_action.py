...
import PyKDL
import actionlib
import ist_tools.msg
from action import CheckServer
...
class CheckServer():
    ...
    #_goal = ist_tools.msg.check_msgGoal()
    _feedback = ist_tools.msg.check_msgFeedback()
    _result = ist_tools.msg.check_msgResult()

    def __init__(self):
        
        #print("loginfo: CheckServer init()")
        self._as = actionlib.SimpleActionServer('check_server', ist_tools.msg.check_msgAction, self.server_callback, False)
        self._as.start()
        self._ac = actionlib.SimpleActionClient('check_server', ist_tools.msg.check_msgAction)
    ...
    def topic_callback(self,msg_data):
        ...
        if _msg[0].upper() == "STOP":
            ...
            goal_pub = ist_tools.msg.check_msgGoal(method="STOP",imu="FALSE", distance=0,angule=0, vel=0, error=10)
        ...
        if check_flag:
            if _msg[0] in ("LINE","LINEAR"):
                goal_pub = ist_tools.msg.check_msgGoal(method=_msg[0],imu=_msg[1],distance=_msg[2],angule=0,vel=_msg[3],error=_msg[4])
            elif _msg[0] in ("ANGULE","ANGULAR"):
                goal_pub = ist_tools.msg.check_msgGoal(method=_msg[0],imu=_msg[1],distance=0,angule=_msg[2],vel=_msg[3],error=_msg[4])
            else:
                goal_pub = ist_tools.msg.check_msgGoal(method=_msg[0],imu=_msg[1],distance=_msg[2],angule=_msg[3],vel=_msg[4],error=_msg[5])
            self._ac.wait_for_server()
            self._ac.send_goal(goal_pub)
        else:
